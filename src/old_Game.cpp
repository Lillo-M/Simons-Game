...
Game::Game()
{
	...
	pAux = new Entities::Characters::Enemies::Warrior(sf::Vector2f(250.f, 200.f));
	if(pAux)
	...
	player = new Entities::Characters::Player(sf::Vector2f(500.f, 592.f));
	Dentities.insert_back(player);
	eManager.setpPlayer(player);
	for (int i = 0; i < 10; i++)
	{
		pAux = new Entities::Obstacles::Ground(sf::Vector2f(64.f + i * 128.f, 656.f));
		if(pAux)
	...
	pAux = new Entities::Obstacles::Ground(sf::Vector2f(300.f, 400.f));
	if(pAux)
		Sentities.insert_back(pAux);
	else
	{
	...
	pAux = new Entities::Obstacles::Ground(sf::Vector2f(1000.f, 400.f));
	if(pAux)
		Sentities.insert_back(pAux);
	else
	{
	...
	eManager.setpPlayer(player);
	CManager.setSList(Sentities);
	CManager.setDList(Dentities);
	Executar();
}
